...
newtype SegTree v n = SegTree (FingerTree (SegMeasure v n) (Segment Closed v n))
  deriving (Eq, Ord, Show, Monoid, FT.Measured (SegMeasure v n))
...
instance (Metric v, OrderedField n) => Transformable (SegTree v n) where
  transform t = over _Wrapped' (FT.fmap' $ transform t)
...
instance (Metric v, OrderedField n, Real n) => Parametric (SegTree v n) where
  atParam t p = offset . fst $ splitAtParam t p

instance Num n => DomainBounds (SegTree v n)

instance (Metric v, OrderedField n, Real n) => EndValues (SegTree v n)
...
trailMeasure :: (Metric v, OrderedField n, SegMeasure v n :>: m, FT.Measured (SegMeasure v n) t)
             => a -> (m -> a) -> t -> a
...
--   @Trail@s...)
numSegs :: (OrderedField n, Num c, Metric v, FT.Measured (SegMeasure v n) a)
        => a -> c
numSegs = fromIntegral . trailMeasure 0 (getSum . op SegCount)

-- | Compute the total offset of anything measured by 'SegMeasure'.
offset :: (OrderedField n, Metric v, FT.Measured (SegMeasure v n) t)
       => t -> v n
...
instance (Metric v, OrderedField n) => AsEmpty (Trail' Line v n) where
  _Empty = nearly emptyLine $ \(Line (SegTree ft)) -> FT.null ft
...
instance (Metric v, OrderedField n, Real n)
  ...
  reverseDomain = undefined -- reverseLine
...
instance (Metric v, OrderedField n, Real n) => Parametric (GetSegment (Trail v n)) where
...
instance (Metric v, OrderedField n) => AsEmpty (Trail v n) where
  _Empty = nearly mempty undefined -- isTrailEmpty
...
onLineSegments f = lineFromSegments . f . undefined -- lineSegments
...
trailSegments = withLine undefined -- lineSegments
...
instance (Metric v, OrderedField n) => Reversing (Located (Trail' l v n)) where
  reversing l@(Loc _ Line {}) = reversing l
  reversing l@(Loc _ Loop {}) = reversing l
